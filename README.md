...
师兄说建议用 Java 的 `Apache POI`，但我刚学了 Golang 一星期，就想用 Golang 做！！！死磕~~~
### 意义
...
* 明天要做：看一下标题内容提取是否有误；把标题与段落内容加上层级关系；现学 `gorm` ，数据入库

## 二、开发问题与解决方案记录
### 1.文件相对绝对路径
Go 的环境路径和项目路径不一致，所以要获取代码文件所在的路径，使用了 `/util/fileUtil.go` 的 `GetRunPath()`
...
正常的格式应该是 `Heading1` 这种（经由 `/parse/parseTry.go` 中的 `TestNewDocGooxml` 测试，确保正确）  
